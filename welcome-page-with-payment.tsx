...
import { ArrowRight, Package, Truck, Plane, Ship, CreditCard } from "lucide-react"
...
function Scene() {
  return (
    <>
      ...
      <Float speed={1.5} rotationIntensity={1} floatIntensity={1} floatingRange={[-0.5, 0.5]}></Float>
    </>
  )
}
...
const iconVariants = {
  hover: (custom: string) => ({
    y: [-3, 3, -3],
    rotate:
      custom === "Plane" ? [-10, 10, -10] : custom === "Ship" ? [-5, 5, -5] : custom === "Truck" ? [-2, 2, -2] : 0,
    transition: {
      duration: 2,
        ...
        >
          ...
          >
            {features.map((feature, index) => (
              ...
              >
                <motion.div custom={feature.label} variants={iconVariants} whileHover="hover">
